...
- `SCAN-CONFIG-UNPARSED` at `high`, for a config that names one of these shapes and whose structure could not be read — because it will not parse, or because it is nested past what the decoder will walk. The text pass has read every line of it and does not cover the gap: its patterns match within one line and a config splits a command across several, which is the whole reason this pass exists. So the declaration in there is the one thing nothing has looked at, and the verdict floor is `review` and exit `1` — the same rule a skipped vendor tree gets. Comments and trailing commas parse, string-aware rather than by regex, because the agent's own parser accepts them and a value holding `//` or `, ]` is ordinary.
