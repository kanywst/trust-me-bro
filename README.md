...
**A hook is a command nobody invokes.** `hooks/hooks.json` — or the same block inside a plugin manifest or a `settings.json` — binds a command to one of the agent's own events. Install the plugin and the command runs when the agent reaches that event, before you see the result, whether or not you ever use the skill. So the declaration itself is the finding, `HOOK-DECLARED` at **high**, and the command it binds is then scanned like any other command. A plugin that points at a hook file instead of declaring one (`"hooks": "./hooks/hooks.json"`) declares nothing here, provided the file it names is one this scan actually read. Existing is not the same as read, so the check is whether the walk would have reached it: a pointer that leaves the target, names nothing at all, goes through a symlink, or lands inside a directory the walk skips is `HOOK-POINTER-UNREAD` at **high**: the hook still registers when the plugin is installed, and the commands in it are then the thing nothing here has looked at. A pointer is only harmless when the thing it points at was read too.
...
The shapes are recognised by shape, not by filename — including the file's own extension. Where a hook block lives has already moved from `settings.json` to a plugin manifest to `hooks/hooks.json`, and a scanner pinned to today's filenames goes quiet the next time it moves. A pointer names an arbitrary path, so requiring `.json` would just mean aiming `"hooks"` at `hooks.txt`. Any file that opens as a JSON document and names one of these keys is read as one, whatever it is called. Comments and trailing commas parse, because the agent's own parser accepts them. A config that names one of these shapes and whose structure could not be read — it will not parse, or it is nested deeper than the decoder will walk — is `SCAN-CONFIG-UNPARSED` at **high**, which puts the verdict at **REVIEW** and exit `1`. The text pass has read every line of it, but its patterns match within one line and a config splits a command across several — which is the whole reason this pass exists — so the declaration in there is the one thing nothing has looked at. Same rule as a skipped vendor tree: the one place nothing looked does not get to come back at exit `0`.
